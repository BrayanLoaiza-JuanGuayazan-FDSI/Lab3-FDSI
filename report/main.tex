...
\texttt{risk-register.md} contiene el detalle completo de activos, actores,
límites de confianza, superficie de ataque y las seis hipótesis STRIDE.
...
funciona antes del despliegue en el servidor.
...
\begin{figure}[H]
    ...
    \label{fig:git_log}
\end{figure}
% PENDIENTE: el historial fue reestructurado (commits más granulares) despues
% de tomar la captura images/git-log.png; volver a ejecutar "git log --oneline -5"
% en el repositorio ya actualizado y reemplazar la captura antes de compilar.
% Mensajes esperados en los ultimos cinco commits (los hashes cambian con cada
% reescritura del historial, no fijarlos aqui):
% docs(threat-model): confirm CORS/proxy fix with browser retest evidence
% docs(readme): update published URL, security limitations and delivery status
% fix(frontend): consume alerts API via nginx proxy to avoid CORS block
% fix(deploy): install Python deps with --break-system-packages, run services as owner
% docs(threat-model): add Parte I risk register and evidence structure
...
Dado que el equipo de desarrollo (Windows) no cuenta con Python instalado, la
prueba equivalente de \texttt{http.server} se ejecutó sirviendo el frontend a
través de Nginx en la VM Ubuntu (que sí trae \texttt{python3} de fábrica),
verificando el mismo comportamiento esperado:

\begin{lstlisting}[language=bash]
curl -I http://localhost/       # en la VM, vía Nginx
curl http://localhost/
\end{lstlisting}
...
\begin{figure}[H]
    ...
    \caption{Prueba de disponibilidad del frontend (HTTP 200).}
...
\end{lstlisting}
Evidencia de ejecución local: \texttt{2026-09-14T02:35:31Z}. Evidencia de
...
\texttt{lab3-fdsi-ubuntu}, IP \texttt{192.168.17.129}.
...
nginx -v
\end{lstlisting}
Versión instalada: \texttt{nginx/1.28.3 (Ubuntu)}.
...
\small
\begin{tabular}{|p{1.7cm}|p{1.5cm}|p{1.7cm}|p{2.2cm}|}
\hline
\textbf{Prueba} &
\textbf{Esperado} &
\textbf{Obtenido} &
...
\hline
\texttt{pip3 install} & Instalación OK & \texttt{externally-managed-\linebreak environment} & Python 3.14/Ubuntu 26.04 (PEP 668); corregido con \texttt{--break-system-packages} \\
\hline
...
del sistema. El detalle completo se encuentra en \texttt{risk-register.md}.
...
La Tabla~\ref{tab:stride} presenta una hipótesis por cada una de las seis
categorías STRIDE, derivada directamente del código de \texttt{alerts-api} y
\texttt{actions-api} (no de un ejemplo genérico).

\clearpage
\begin{table*}[t]
\centering
\caption{Modelo inicial de amenazas STRIDE --- CrowdStrike Falcon Alert Simulator}
\label{tab:stride}
\scriptsize
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{|p{0.5cm}|p{2.1cm}|p{3.9cm}|p{3.9cm}|p{2.3cm}|}
\hline
\textbf{ID} & \textbf{Categoría} & \textbf{Hipótesis técnica} & \textbf{Evidencia / mitigación} & \textbf{Estado} \\
\hline

H1 & Spoofing &
Sin autenticación, cualquiera registra una acción suplantando a un analista
(\texttt{analyst} es texto libre en el POST). &
\texttt{POST /actions} con \texttt{analyst} arbitrario se acepta sin verificar
identidad. Mitigación: autenticación de analistas en Lab 4. &
Pendiente Lab 4 \\
\hline

H2 & Tampering &
Sin TLS ni validación de esquema, el contenido de una alerta puede alterarse
en tránsito o al crearla con campos arbitrarios. &
\texttt{POST /alerts} acepta \texttt{severity}/\texttt{host}/\texttt{ioc}
arbitrarios sin validar. Mitigación: validación de esquema + HTTPS. &
Pendiente Lab 4 \\
\hline

H3 & Repudiation &
La bitácora de acciones no está ligada a una identidad autenticada; una
acción puede negarse o atribuirse a otro analista. &
\texttt{logging.info} solo registra lo que el cliente declara, sin sesión que
lo respalde. Mitigación: auditoría firmada ligada a sesión. &
Pendiente Lab 4 \\
\hline

H4 & Information Disclosure &
\texttt{/debug/env} expone variables de entorno del servidor; los headers de
Flask/Nginx revelan tecnología y versión. &
\texttt{curl -i .../debug/env} devuelve el entorno completo del proceso.
Mitigación: \texttt{server\_tokens off}, retirar/proteger el endpoint. &
Mitigado parcialmente \\
\hline

H5 & Denial of Service &
El servidor Flask de desarrollo (single-threaded, no apto para producción) y
los POST sin límite permiten crecimiento no acotado de \texttt{ALERTS}/\texttt{ACTIONS}
en memoria. &
Ráfaga de \texttt{POST /alerts} crece la lista sin límite. Mitigación:
WSGI de producción (gunicorn) + \texttt{limit\_req} en Nginx. &
Pendiente Lab 4/5 \\
\hline

H6 & Elevation of Privilege &
Sin modelo de autorización, cualquier cliente anónimo ejecuta acciones
privilegiadas (cerrar/escalar alertas CRITICAL) igual que un analista real. &
\texttt{POST /actions} con \texttt{action=close} cambia el estado de una
alerta CRITICAL sin control de rol. Mitigación: RBAC en Lab 4. &
Pendiente Lab 4 \\
\hline
...
\begin{itemize}
    \item Implementar HTTPS/TLS y autenticación de analistas (Laboratorio 4).
    \item Restringir el bind de los microservicios a \texttt{127.0.0.1} y
    dejar que solo Nginx los exponga.
    \item Retirar o proteger \texttt{/debug/env}; aplicar \texttt{server\_tokens off}.
    \item Servir con un WSGI de producción y limitar tasa de solicitudes.
    \item Incorporar RBAC y validación de esquema en las siguientes fases.
\end{itemize}
