...
\large \textit{Prototipo: CrowdStrike Falcon Alert Simulator}
}

\author{
Brayan Loaiza Leal,
Juan Sebastián Guayazán Clavijo
\thanks{Primera entrega (Parte I)}
...
\begin{figure}[H]
    ...
    \label{fig:git_log}
\end{figure}
% NOTA: cada commit posterior a esta captura la vuelve desactualizada (git log
% siempre muestra los ultimos 5). Retomarla como ULTIMO paso, justo antes de
% exportar el PDF final, para que coincida con el commit realmente evaluado.
...
La Figura~\ref{fig:browser} muestra el dashboard ya simplificado, con las
cinco alertas y los contadores por severidad correctamente cargados tras la
corrección del bloqueo por CORS descrita en la Sección~\ref{sec:amenazas}
(hallazgo L3). Verificación tomada en un entorno local equivalente
(\texttt{alerts-api}, \texttt{actions-api} y el proxy sirviendo el mismo
\texttt{frontend/index.html} del repositorio); la evidencia contra la VM
publicada (\texttt{192.168.17.129}) se actualiza en el próximo redespliegue.
...
\hline
\texttt{pip3 install} & Instalación OK & \url{externally-managed-environment} & Python 3.14/Ubuntu 26.04 (PEP 668); corregido con \texttt{--break-system-packages} \\
\hline
Arranque \url{alerts-api/actions-api} & \texttt{active (running)} & \texttt{status=200/CHDIR} & \texttt{User=www-data} sin permiso sobre \texttt{\$HOME} (750); corregido a usuario dueño del repo \\
\hline
...
\begin{itemize}
    ...
    \url{https://pruebacorreoescuelaingeduco-my.sharepoint.com/:x:/g/personal/juan_guayazan-c_mail_escuelaing_edu_co/IQCsLx8MX5sySIz1uJj1FwUkAQMTC_xtYA3lVGlnFN1Ojvc?e=U0kC20}
    (también versionado en el repositorio, en
    \texttt{threat-model/threat-model.xlsx})
\end{itemize}
